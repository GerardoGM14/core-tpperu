...
const DOCS = [
  { id: 'DOC-001', name: 'Itinerario · Tarapoto 7d/6n',              type: 'PDF', size: '1.4 MB', pkg: 'PKG-001', updated: '04 may 2026', sent: 142, status: 'activo' },
  { id: 'DOC-002', name: 'Punto de encuentro · Aeropuerto TPP',        type: 'IMG', size: '620 KB', pkg: 'PKG-001', updated: '02 may 2026', sent: 142, status: 'activo' },
  { id: 'DOC-003', name: 'Recomendaciones de equipaje',                type: 'PDF', size: '380 KB', pkg: 'PKG-001', updated: '18 abr 2026', sent: 142, status: 'activo' },
  { id: 'DOC-004', name: 'Contactos de emergencia · Guía Luis A.',     type: 'PDF', size: '180 KB', pkg: 'PKG-001', updated: '18 abr 2026', sent: 142, status: 'activo' },
  { id: 'DOC-005', name: 'Flyer Cataratas Ahuashiyacu',                type: 'IMG', size: '1.1 MB', pkg: 'PKG-002', updated: '30 abr 2026', sent: 87,  status: 'activo' },
  { id: 'DOC-006', name: 'Mapa ruta Laguna Azul',                      type: 'IMG', size: '880 KB', pkg: 'PKG-003', updated: '27 abr 2026', sent: 27,  status: 'activo' },
  { id: 'DOC-007', name: 'Itinerario · Alto Mayo 3d',                  type: 'PDF', size: '1.0 MB', pkg: 'PKG-006', updated: '06 may 2026', sent: 0,   status: 'borrador' },
  { id: 'DOC-008', name: 'Voucher reserva (plantilla)',                 type: 'PDF', size: '120 KB', pkg: 'GLOBAL',  updated: '01 may 2026', sent: 412, status: 'activo' },
];

export function Documentos() {
  const [sel, setSel] = React.useState('DOC-001');
  const [filter, setFilter] = React.useState('todos');
  const filtered = DOCS.filter(d => filter === 'todos' || d.type.toLowerCase() === filter);
    ...
    <div className="view">
      <div className="row between">
        <div>
          ...
          <p className="lead" style={{ marginTop: 4 }}>Itinerarios, mapas, recomendaciones y contactos. Se envían automáticamente por WhatsApp al confirmarse la reserva y antes del tour.</p>
        </div>
        <div className="row" style={{ gap: 8 }}>
          <button className="btn ghost"><Ico.copy />Subir archivo</button>
          <button className="btn"><Ico.plus />Nuevo documento</button>
        </div>
      </div>
      ...
      <div className="grid-stats">
        <div className="stat"><div className="label">Documentos activos</div><div className="value">{DOCS.filter(x => x.status === 'activo').length}</div><div className="delta up"><Ico.up />+2 esta semana</div></div>
        <div className="stat"><div className="label">Envíos automáticos</div><div className="value">{DOCS.reduce((s, x) => s + x.sent, 0).toLocaleString()}</div><div className="delta up"><Ico.up />+87 hoy</div></div>
        <div className="stat"><div className="label">Tasa de lectura WhatsApp</div><div className="value">94<span style={{ fontSize: 14, color: 'var(--muted)' }}>%</span></div><div className="delta up"><Ico.up />+1.2 pts</div></div>
        <div className="stat"><div className="label">Errores de envío</div><div className="value">3</div><div className="delta down"><Ico.down />–4 vs. ayer</div></div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 12, alignItems: 'start' }}>
            ...
            <div className="tabs" style={{ border: 0, marginBottom: -13, marginTop: -12 }}>
              {['todos', 'pdf', 'img'].map(k => (
                <button key={k} className={'tab ' + (filter === k ? 'active' : '')} onClick={() => setFilter(k)}>
                  {k.toUpperCase()}<span className="mono" style={{ marginLeft: 6, fontSize: 10, color: 'var(--muted)' }}>{k === 'todos' ? DOCS.length : DOCS.filter(d => d.type.toLowerCase() === k).length}</span>
                </button>
              ))}
            </div>
            <div style={{ flex: 1 }} />
            <div className="topbar-search" style={{ width: 200 }}><Ico.search /><input placeholder="Buscar archivo..." /></div>
          </div>
          <div className="card-b flush">
            <table className="t">
              <thead><tr><th>ID</th><th>Documento</th><th>Tipo</th><th>Paquete asociado</th><th style={{ textAlign: 'right' }}>Envíos</th><th>Actualizado</th><th>Estado</th></tr></thead>
              <tbody>
                {filtered.map(x => (
                  <tr key={x.id} className={sel === x.id ? 'selected' : ''} onClick={() => setSel(x.id)} style={{ cursor: 'pointer' }}>
                    <td><span className="cell-id">{x.id}</span></td>
                    <td>
                      <div className="row" style={{ gap: 8 }}>
                        <div className="imgph" style={{ width: 32, height: 32, borderRadius: 3, fontSize: 9 }}>{x.type}</div>
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 500 }}>{x.name}</div>
                          <div className="cell-id">{x.size}</div>
                        </div>
                      </div>
                    </td>
                    <td><span className="pill mono" style={{ fontSize: 10 }}>{x.type}</span></td>
                    <td className="cell-id">{x.pkg}</td>
                    <td className="cell-num" style={{ textAlign: 'right' }}>{x.sent}</td>
                    <td className="cell-id">{x.updated}</td>
                    <td>{x.status === 'activo' ? <span className="pill good"><span className="d" />activo</span> : <span className="pill"><span className="d" style={{ background: 'var(--muted)' }} />borrador</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {d && (
          <div className="card" style={{ position: 'sticky', top: 0 }}>
            <div className="card-h"><h2 className="h2">{d.id}</h2><span className="pill mono" style={{ fontSize: 10 }}>{d.type}</span></div>
            <div className="card-b" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div className="imgph" style={{ height: 160, borderRadius: 4, fontSize: 11 }}>vista previa · {d.name}</div>
              <div className="field"><label>Nombre</label><input className="input" defaultValue={d.name} /></div>
              <div className="field"><label>Paquete asociado</label>
                <select className="input" defaultValue={d.pkg}>
                  <option value="GLOBAL">Todos los paquetes</option>
                  <option value="PKG-001">PKG-001 · Tarapoto 7d/6n</option>
                  <option value="PKG-002">PKG-002 · Cataratas Ahuashiyacu</option>
                  <option value="PKG-003">PKG-003 · Laguna Azul + Sauce</option>
                  <option value="PKG-006">PKG-006 · Alto Mayo 3d</option>
                </select>
              </div>
              <div className="field"><label>Cuándo enviar al cliente</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    ['Al confirmar la reserva', true],
                    ['48 horas antes del tour', true],
                    ['24 horas antes del tour', false],
                    ['12 horas antes del tour', true],
                    ['Después del tour', false],
                  ].map(([lbl, on], i) => (
                    <label key={i} className="row" style={{ gap: 8, fontSize: 12, padding: '4px 0' }}>
              ...
              <div className="field"><label>Plantilla de mensaje</label>
                <select className="input"><option>WSP_DOC_ITINERARIO_v2</option><option>WSP_DOC_PUNTO_ENCUENTRO</option></select>
              </div>
              <div className="row" style={{ gap: 6 }}>
                <button className="btn ghost" style={{ flex: 1 }}><Ico.eye />Vista previa</button>
                <button className="btn" style={{ flex: 1 }}><Ico.send />Probar envío</button>
              </div>
            </div>
          </div>
        )}
